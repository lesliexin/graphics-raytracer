...
bool SceneNode::intersect(Ray &ray, float closet, float fart, Intersection &intersection) {
	bool isIntersect = false;
	vec4 oAug = vec4(ray.origin, 1.0);
	vec4 dAug = vec4(ray.direction, 1.0);
	Ray transform(vec3(invtrans*oAug), vec3(invtrans*dAug));

	Intersection tempIntersection;
	for (SceneNode* child : children) {
		if (child->intersect(transform, closet, fart, tempIntersection)) {
			isIntersect = true;
			fart = tempIntersection.t;
			intersection = tempIntersection;
		} 
	}
